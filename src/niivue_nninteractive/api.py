...
async def segment_image(
    ...
    """
    logger.info(
        f"Segmentation request received - user_id: {user_id}, has_image: {image is not None}"
    )
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Invalid scribbles format: {str(e)}"
        )
...
async def process_uploaded_image(image: UploadFile) -> Tuple[np.ndarray, sitk.Image]:
    ...
    if not image.filename.endswith((".nii", ".nii.gz")):
        ...
        raise HTTPException(
            status_code=400, detail="Only NIfTI files (.nii, .nii.gz) are supported"
        )
    ...
    try:
        ...
        logger.error(f"Traceback: {traceback.format_exc()}")
        raise HTTPException(
            status_code=400, detail=f"Failed to load NIfTI image: {str(e)}"
        )


def perform_segmentation(
    img_array: np.ndarray, scribbles: List[ScribbleCoordinate]
) -> torch.Tensor:
    ...
    for i, scribble in enumerate(scribbles[:5]):  # Log first 5 scribbles
        logger.info(
            f"Scribble {i}: x={scribble.x}, y={scribble.y}, z={scribble.z}, positive={scribble.is_positive}"
        )
    ...
    for i, scribble in enumerate(scribbles):
        ...
        if (
            0 <= scribble.x < img_dims[2]
            and 0 <= scribble.y < img_dims[1]
            and 0 <= scribble.z < img_dims[0]
        ):
            valid_scribbles.append(scribble)
        else:
            logger.warning(
                f"Scribble {i} out of bounds: x={scribble.x}, y={scribble.y}, z={scribble.z}, bounds=(z={img_dims[0]}, y={img_dims[1]}, x={img_dims[2]})"
            )
    ...
    if positive_scribbles:
        logger.info(
            f"Creating positive scribble image for {len(positive_scribbles)} scribbles"
        )
        ...
        for i, scribble in enumerate(positive_scribbles):
            ...
            for dx in range(-scribble_thickness, scribble_thickness + 1):
                for dy in range(-scribble_thickness, scribble_thickness + 1):
                    ...
                    if (
                        0 <= x < positive_scribble_image.shape[2]
                        and 0 <= y < positive_scribble_image.shape[1]
                        and 0 <= z < positive_scribble_image.shape[0]
                    ):
                        positive_scribble_image[z, y, x] = 1
                    else:
                        logger.debug(
                            f"Scribble pixel out of bounds: x={x}, y={y}, z={z}, shape={positive_scribble_image.shape}"
                        )
            ...
            if i < len(positive_scribbles) - 1:
                ...
                if scribble.z == next_scribble.z:
                    steps = max(
                        abs(next_scribble.x - scribble.x),
                        abs(next_scribble.y - scribble.y),
                    )
                    if steps > 0:
                        for step in range(steps + 1):
                            ...
                            for dx in range(
                                -scribble_thickness // 2, scribble_thickness // 2 + 1
                            ):
                                for dy in range(
                                    -scribble_thickness // 2,
                                    scribble_thickness // 2 + 1,
                                ):
                                    px = x + dx
                                    py = y + dy
                                    if (
                                        0 <= px < positive_scribble_image.shape[2]
                                        and 0 <= py < positive_scribble_image.shape[1]
                                        and 0
                                        <= scribble.z
                                        < positive_scribble_image.shape[0]
                                    ):
                                        positive_scribble_image[scribble.z, py, px] = 1

        # Add positive scribble interaction
        logger.info(
            f"Adding positive scribble interaction - scribble pixels: {np.sum(positive_scribble_image)}"
        )
        model_session.add_scribble_interaction(
            positive_scribble_image, include_interaction=True
        )

    # Create negative scribble image if we have negative scribbles
    if negative_scribbles:
        logger.info(
            f"Creating negative scribble image for {len(negative_scribbles)} scribbles"
        )
        ...
        for scribble in negative_scribbles:
            ...
            for dx in range(-scribble_thickness // 2, scribble_thickness // 2 + 1):
                for dy in range(-scribble_thickness // 2, scribble_thickness // 2 + 1):
                    ...
                    # Check bounds and index correctly (array is [z, y, x])
                    if (
                        0 <= x < negative_scribble_image.shape[2]
                        and 0 <= y < negative_scribble_image.shape[1]
                        and 0 <= z < negative_scribble_image.shape[0]
                    ):
                        negative_scribble_image[z, y, x] = 1
                    else:
                        logger.debug(
                            f"Negative scribble pixel out of bounds: x={x}, y={y}, z={z}, shape={negative_scribble_image.shape}"
                        )

        # Add negative scribble interaction with include_interaction=False
        logger.info(
            f"Adding negative scribble interaction - scribble pixels: {np.sum(negative_scribble_image)}"
        )
        model_session.add_scribble_interaction(
            negative_scribble_image, include_interaction=False
        )
